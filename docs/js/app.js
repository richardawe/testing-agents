// AI Agent Workflow Frontend JavaScript for GitHub Pages

// Configuration - GitHub Pages version
const GITHUB_REPO = window.GITHUB_REPO || 'your-username/your-repo';
...
async function triggerWorkflow(task) {
    ...
    if (!owner || !repo || owner === 'your-username') {
        throw new Error('Invalid GitHub repository. Please set GITHUB_REPO in the HTML file.');
    }
...
function updateWorkflowStatus(status) {
    ...
    if (statusElement) {
        statusElement.textContent = `Status: ${status.status || 'running'}...`;
...
async function handleFormSubmit(e) {
    ...
    if (statusMsg) {
        statusMsg.textContent = 'Triggering workflow... This may take a few minutes.';
    ...
    try {
        ...
        // Update status
        if (statusMsg) {
            statusMsg.textContent = 'Workflow triggered. Waiting for results...';
        }
        ...
        if (data.success) {
            ...
            if (statusMsg) {
                statusMsg.textContent = 'Workflow completed successfully!';
